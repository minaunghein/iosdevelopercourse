...
print("last elements(optional): ==>", numbers1.last )

//Vector
//Everthing in Swift is object !!!!!
var test = 5.advanced(by: 3)
print(test)

//3.Collection - Set
var uniqueNumbers1TraditionalArray: Array<Int> = [1,2,3,4,5]
...
var oNum3 =  oNum2
print(oNum3,"Assigned from !")
if oNum1 != nil {  //User != nil to check for value present
    print("Value present")
    let num1 = oNum1! //force unwrapping    // ! convert optional to normal var (non-opt)
    print(num1 )
} else {
    print("Value Absent")
}

if let oNum2 = oNum2 { // optinal binding
    var oNum2Mutable = oNum2
    print(oNum2Mutable) // it is sure that mynumber has value and it is under its local scope within  { }
}
...
//While
//1. MUST initialise
//2. Loop until exit condition
//3. Condition must be met, inc or dec to the index or x

var i = 0 //1
while (i < array.count) { //2 //Analogue
    print(i, array[i])
    i += 2 //3
}
print("\\\\\\\\\\\\")

var myArray = [3,5,7,9,11,13,15]

for e in  myArray {   //for in-loop - direct element access
    print("*",e)  // Digitally
}

for i in 0..<myArray.count { //via indexing
    print(myArray[i])
}

for i in stride(from: 0, to: myArray.count, by: 2) { // to is <
    print("->",myArray[i])
}
for i in stride(from: myArray.count - 1, through: 0, by: -2) { // to is <
    print("->",myArray[i])
}
for i in stride(from: 0, through: myArray.count - 1 , by: 2) { //through is until
    print("->",myArray[i])
}
...
func greet(_ name:String, ver:Float = 4.2 ) {  // func nameofFunction([paramName:TypeofParam [ = defaultValue] ]) {}
    //_ is external param and name is internal param
    print("Hello Swift \(ver) by \(name)")
}

func greet () { //Function overloading => Same name, different para
    print("Good moring")
}
greet()
greet("Min") //_, omit when user aware how value inputed is applied
//DONT use _ when parameters are more than 3

func calculateDropspeed(standardGravity g: Float ){
    var s = 3.1 * g
}

calculateDropspeed(standardGravity: 9.8)
